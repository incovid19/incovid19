        ...
        if source["myGov"][idx] != "yes":
            ...
            if source["StateDataSourceType"][idx] == "json":
                if source["StateCode"][idx] == "MH":
                    try:
                        response = requests.get(source["StateDataURL"][idx] + '/dbd-cases-file?_by=District&_by=Date',timeout=20)
                        if response.status_code == 200:
                ...
                try:
                    ...
                    #     downloadFile(str(date), source["StateCode"][idx], url)
                        ...
                        try:
                            url = 'https://meghalaya.gov.in/sites/default/files/announcement/Daily_Covid_' + date.strftime("%d_%b_%Y.pdf")

...
def getAPData(path):
    ...
    df_json = pd.read_json("/mnt/c/Users/gopis/Desktop/django/incovid19/DistrictMappingMaster.json")
...
def GenerateRawCsv(StateCode,Date,df_districts):
    ...
    "cumulativeTestedNumberForDistrict","last_updated","tested_last_updated_statetested_source_state","notesForState",
...
def ExtractFromHTML(StateCode = "AS",Date = "2021-10-26"):
    try:
        filepath = "/mnt/c/users/gopis/desktop/django/pdf-incovid19/incovid19/INPUT/{0}/{1}.html".format(Date,StateCode)
        if StateCode == "AP":

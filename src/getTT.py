...
def getTT():
    today = (datetime.datetime.now() - timedelta.Timedelta(days= 134 + 0)).date()
    pDate = (datetime.datetime.now() - timedelta.Timedelta(days= 134 + 1)).date()
    cowinDate = (datetime.datetime.now() - timedelta.Timedelta(days= 134 + 2)).date()
    ...
    try:
        for idx in source.index:
            if source["StateCode"][idx] != "TT":
                ...
                # temp_df["notesForState"] = "Since there are no updates for 4th August from MoHFW the portal reflects data of 03rd August 2023"

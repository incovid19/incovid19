...
def getTT():
    today = (datetime.datetime.now() - timedelta.Timedelta(days= 142 + 0)).date()
    pDate = (datetime.datetime.now() - timedelta.Timedelta(days= 142 + 1)).date()
    cowinDate = (datetime.datetime.now() - timedelta.Timedelta(days= 142 + 2)).date()
    ...
    try:
        for idx in source.index:
            if source["StateCode"][idx] != "TT":
                ...
                # temp_df["notesForState"] = "Since there are no updates for 2nd August from MoHFW the portal reflects data of 01st August 2023"

...
def getTT():
    today = (datetime.datetime.now() - timedelta.Timedelta(days= 12 + 0)).date()
    pDate = (datetime.datetime.now() - timedelta.Timedelta(days= 12 + 1)).date()
    cowinDate = (datetime.datetime.now() - timedelta.Timedelta(days= 12 + 2)).date()
    ...
    try:
        for idx in source.index:
            if source["StateCode"][idx] != "TT":
                ...
                # temp_df["notesForState"] = "Since there are no updates for 27th May from MoHFW the portal reflects data of 26th May 2023"

...
def getTT():
    today = (datetime.datetime.now() - timedelta.Timedelta(days= 2 + 0)).date()
    pDate = (datetime.datetime.now() - timedelta.Timedelta(days= 2 + 1)).date()
    cowinDate = (datetime.datetime.now() - timedelta.Timedelta(days= 2 + 2)).date()
    ...
    try:
        for idx in source.index:
            if source["StateCode"][idx] != "TT":
                ...
                temp_df["notesForState"] = "Since there are no updates for 20th June from MoHFW the portal reflects data of 19th June 2023"

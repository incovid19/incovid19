...
def getTT():
    today = (datetime.datetime.now() - timedelta.Timedelta(days= 5 + 0)).date()
    pDate = (datetime.datetime.now() - timedelta.Timedelta(days= 5 + 1)).date()
    cowinDate = (datetime.datetime.now() - timedelta.Timedelta(days= 5 + 2)).date()
    ...
    try:
        for idx in source.index:
            if source["StateCode"][idx] != "TT":
                ...
                # temp_df["notesForState"] = "Since there are no updates for 22th June from MoHFW the portal reflects data of 21th June 2023"
